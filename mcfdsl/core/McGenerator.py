...
from mcfdsl.core.importManager import ImportManager
from mcfdsl.core.result import Result
from mcfdsl.core.scope import Scope
from mcfdsl.core.symbol import Symbol
from mcfdsl.core.types import ScopeType, SymbolType, Type, BaseType
from mcfdsl.core.utils.typeUtils import TypeUtils
...
class MCGenerator(McFuncDSLVisitor):
    ...
    def visitVarDeclaration(self, ctx: McFuncDSLParser.VarDeclarationContext):
        ...
        if ctx.primary():
            ...
            if var_type == TypeUtils.infer(result):
                symbol.value = result.value
            else:
                raise TypeError(f"需要{var_type}却设置了{TypeUtils.infer(result.value)}")
    ...
    def visitAssignment(self, ctx):
        ...
        # 正确的类型检查
        expr_result_type = TypeUtils.infer(expr_result)
