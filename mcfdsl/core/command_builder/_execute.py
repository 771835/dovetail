...
Axes = Literal["x", "y", "z", "xy", "xz", "yz", "xyz", "yx", "yzx", "zy", "zxy", "zyx"]
Anchor = Literal["eyes", "feet"]
OnRelationship = Literal["attacker", "controller", "leasher", "origin", "owner", "passengers", "target", "vehicle"]
HeightmapType = Literal["motion_blocking", "motion_blocking_no_leaves", "ocean_floor", "world_surface"]
...
class ExecuteBuilder:
    ...
    def _add_conditional_part(self, condition_type: Literal["if", "unless"], part: str) -> 'ExecuteBuilder':
    ...
    def if_blocks(self, start: str, end: str, destination: str, mode: BlocksMode) -> 'ExecuteBuilder':
        """Compares two block regions (start..end with destination origin)."""
        return self._add_conditional_part("if", f"blocks {start} {end} {destination} {mode}")
    def unless_blocks(self, start: str, end: str, destination: str, mode: BlocksMode) -> 'ExecuteBuilder':
        """Tests if two block regions do NOT match."""
        return self._add_conditional_part("unless", f"blocks {start} {end} {destination} {mode}")
    ...
    def if_data_block(self, source_pos: str, path: str) -> 'ExecuteBuilder':
        ...
        return self._add_conditional_part("if", f"data block {source_pos} {path}")
    def unless_data_block(self, source_pos: str, path: str) -> 'ExecuteBuilder':
        """Tests for the non-existence of NBT data at a specific path on a block."""
        return self._add_conditional_part("unless", f"data block {source_pos} {path}")
    ...
    def unless_data_entity(self, source: str, path: str) -> 'ExecuteBuilder':
        ...
        return self._add_conditional_part("unless", f"data entity {source} {path}")
    ...
    def if_data_storage(self, source: str, path: str) -> 'ExecuteBuilder':
        ...
        return self._add_conditional_part("if", f"data storage {source} {path}")
    def unless_data_storage(self, source: str, path: str) -> 'ExecuteBuilder':
        ...
        return self._add_conditional_part("unless", f"data storage {source} {path}")
    ...
    # Item conditions
    def if_items_block(self, pos: str, slots: str, item_predicate: str) -> 'ExecuteBuilder':
        """Tests for items matching a predicate in a block inventory slot(s)."""
        return self._add_conditional_part("if", f"items block {pos} {slots} {item_predicate}")
    def unless_items_block(self, pos: str, slots: str, item_predicate: str) -> 'ExecuteBuilder':
        """Tests for the absence of items matching a predicate in a block inventory slot(s)."""
        return self._add_conditional_part("unless", f"items block {pos} {slots} {item_predicate}")

    def if_items_entity(self, entities: str, slots: str, item_predicate: str) -> 'ExecuteBuilder':
        """Tests for items matching a predicate in an entity inventory slot(s)."""
        return self._add_conditional_part("if", f"items entity {entities} {slots} {item_predicate}")
    def unless_items_entity(self, entities: str, slots: str, item_predicate: str) -> 'ExecuteBuilder':
        """Tests for the absence of items matching a predicate in an entity inventory slot(s)."""
        return self._add_conditional_part("unless", f"items entity {entities} {slots} {item_predicate}")
    ...
    # Score conditions
    def if_score_compare(self, target: str, target_objective: str, operation: ScoreOperation, source: str, source_objective: str) -> 'ExecuteBuilder':
        """Compares two scores."""
        return self._add_conditional_part("if", f"score {target} {target_objective} {operation} {source} {source_objective}")
    def unless_score_compare(self, target: str, target_objective: str, operation: ScoreOperation, source: str, source_objective: str) -> 'ExecuteBuilder':
        """Tests if two scores do NOT match the comparison."""
        return self._add_conditional_part("unless", f"score {target} {target_objective} {operation} {source} {source_objective}")

    def if_score_matches(self, target: str, target_objective: str, range_: str) -> 'ExecuteBuilder':
        """Tests if a score matches a numerical range."""
        # Using range_ because 'range' is a built-in function in Python
        return self._add_conditional_part("if", f"score {target} {target_objective} matches {range_}")
    def unless_score_matches(self, target: str, target_objective: str, range_: str) -> 'ExecuteBuilder':
        """Tests if a score does NOT match a numerical range."""
        return self._add_conditional_part("unless", f"score {target} {target_objective} matches {range_}")
    ...
    def _add_store_part(self, store_type: Literal["result", "success"], part: str) -> 'ExecuteBuilder':
         """Helper to add a store part and return self."""
         return self._add_part(f"store {store_type} {part}")

    # store block
    def store_result_block(self, target_pos: str, path: str, type: StoreType, scale: float) -> 'ExecuteBuilder':
        """Stores the numerical result of the command into a block's NBT."""
        return self._add_store_part("result", f"block {target_pos} {path} {type} {scale}")
    def store_success_block(self, target_pos: str, path: str, type: StoreType, scale: float) -> 'ExecuteBuilder':
        """Stores the success of the command (1 or 0) into a block's NBT."""
        return self._add_store_part("success", f"block {target_pos} {path} {type} {scale}")

    # store bossbar
    def store_result_bossbar(self, id: str, type: BossbarStoreType) -> 'ExecuteBuilder':
        """Stores the numerical result of the command into a bossbar's value or max."""
        return self._add_store_part("result", f"bossbar {id} {type}")
    def store_success_bossbar(self, id: str, type: BossbarStoreType) -> 'ExecuteBuilder':
        """Stores the success of the command (1 or 0) into a bossbar's value or max."""
        return self._add_store_part("success", f"bossbar {id} {type}")

    # store entity
    def store_result_entity(self, target: str, path: str, type: StoreType, scale: float) -> 'ExecuteBuilder':
        """Stores the numerical result of the command into an entity's NBT."""
        return self._add_store_part("result", f"entity {target} {path} {type} {scale}")
    def store_success_entity(self, target: str, path: str, type: StoreType, scale: float) -> 'ExecuteBuilder':
        """Stores the success of the command (1 or 0) into an entity's NBT."""
        return self._add_store_part("success", f"entity {target} {path} {type} {scale}")

    # store score
    def store_result_score(self, targets: str, objective: str) -> 'ExecuteBuilder':
        """Stores the numerical result of the command into a score."""
        return self._add_store_part("result", f"score {targets} {objective}")
    def store_success_score(self, targets: str, objective: str) -> 'ExecuteBuilder':
        """Stores the success of the command (1 or 0) into a score."""
        return self._add_store_part("success", f"score {targets} {objective}")

    # store storage
    def store_result_storage(self, target: str, path: str, type: StoreType, scale: float) -> 'ExecuteBuilder':
        """Stores the numerical result of the command into a storage's NBT."""
        return self._add_store_part("result", f"storage {target} {path} {type} {scale}")
    def store_success_storage(self, target: str, path: str, type: StoreType, scale: float) -> 'ExecuteBuilder':
        """Stores the success of the command (1 or 0) into a storage's NBT."""
        return self._add_store_part("success", f"storage {target} {path} {type} {scale}")
    ...
    # Alias for __str__ to allow printing the command at any stage (useful for debugging)
